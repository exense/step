...
public class YamlAlertingRuleConditionDeserializer extends JsonDeserializer<AlertingRuleCondition> {
    ...
    public AlertingRuleCondition deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        return new NamedEntityYamlDeserializer<>(AlertingRuleCondition.class) {
            @Override
            protected String resolveTargetClassNameByYamlName(String yamlName) {
                Set<Class<?>> classes = CachedAnnotationScanner.getClassesWithAnnotation(
                        AutomationPackageRuleCondition.LOCATION,
                        AutomationPackageRuleCondition.class,
                        Thread.currentThread().getContextClassLoader()
                );
                for (Class<?> cl : classes) {
                    if (cl.getSimpleName().equals(yamlName)) {
                        return cl.getName();
                    }
                }
                return null;
