...
public class YamlKeywordsDeserializer  extends JsonDeserializer<YamlKeywords> {
    ...
    @Override
    public YamlKeywords deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
        ...
        if (node.isArray()) {
            ...
            for (JsonNode keywordNode : arrayNode) {
                ...
                functions.add(jsonParser.getCodec().treeToValue(techKeyword, Function.class));
            }
        }
        return new YamlKeywords(functions);
    }
