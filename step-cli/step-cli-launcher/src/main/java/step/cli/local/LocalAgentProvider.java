 ...
 * machine. Implementations are discovered with the {@link java.util.ServiceLoader}, which is what keeps the agent
 * types the current distribution doesn't ship out of the picture: an agent type is offered for local execution if and
 * only if a provider for it is on the classpath and reports itself as {@link #isAvailable() available}.
 * <p>
 * None of the agents bring their own runtime: the Java agent runs on a JVM, the Node.js agent on Node.js and the .NET
 * agent on .NET. Only the Java one is therefore available by construction, the CLI starting it being itself a Java
 * application; the others depend on what the developer machine happens to provide. Deciding that, and doing it
