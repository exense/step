 ...
 */
public class CachedCRUDAccessor<T extends AbstractIdentifiableObject> implements CRUDAccessor<T> {

	private final InMemoryCRUDAccessor<T> cache = new InMemoryCRUDAccessor<>();

	private final CRUDAccessor<T> underlyingAccessor;

	/**
	 * @param underlyingAccessor the {@link Accessor} from which the entities should
	 *                           be loaded
	 */
	public CachedCRUDAccessor(CRUDAccessor<T> underlyingAccessor) {
		super();
