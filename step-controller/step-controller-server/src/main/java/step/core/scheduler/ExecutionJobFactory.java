...
public class ExecutionJobFactory implements JobFactory {
    ...
    @Override
    public Job newJob(TriggerFiredBundle arg0, Scheduler arg1) {
        ...
        String executorClassName = configuration.getProperty("controller.execution.job.classname", ExecutionJob.class.getName());
        try {
            Class<?> aClass = Class.forName(executorClassName);
            Constructor<?> constructor = aClass.getConstructor(ExecutionEngine.class, String.class);
            return (Job) constructor.newInstance(executionEngine, executionID);
        } catch (Exception e) {
            throw new RuntimeException("Error while initializing executor " + executorClassName, e);
        }
    }
}
