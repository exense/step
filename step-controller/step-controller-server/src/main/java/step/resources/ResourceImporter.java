...
public class ResourceImporter implements BiConsumer<Object, ImportContext> {
	...
	public void accept(Object t, ImportContext importContext) {
		if(t instanceof Resource) {
			...
			//Revision ID is untouched and still match the ID as it was exported
			//When importing resource we import all revisions of this resource contained in the archive, but we import the one matching the current resource revision at the end
			//In the process we replace all revisions references (old it to new id with the revision IDs generated by resourceManager.saveResourceContent
			String origRevisionId = resource.getCurrentRevisionId().toHexString();
			if(importConfiguration.isOverwrite()) {
				...
				//Revisions were processed and ids replaces in the tmp collection, so we need to get the new ID
			...
			resourceRevisionAbstractAccessor.getResourceRevisionsByResourceId(origResourceId).forEachRemaining(resourceRevision -> {
				...
				if (!tmpCollectionCurrentRevisionId.equals(resourceRevision.getId().toHexString())) {
					importRevision(importContext, resourceRevision, resource, origResourceId, origRevisionId, importConfiguration);
				} else {
			...
			if (currentRevision.get() != null) {
				importRevision(importContext, currentRevision.get(), resource, origResourceId, origRevisionId, importConfiguration);
			} else {
	...
	private void importRevision(ImportContext importContext, ResourceRevision resourceRevision, Resource resource, String origResourceId, String origRevisionId, ImportConfiguration importConfiguration) {
		Path path = Paths.get(importContext.getWorkFolder().getAbsolutePath(), resource.getResourceType(), origResourceId, origRevisionId, resourceRevision.getResourceFileName());
