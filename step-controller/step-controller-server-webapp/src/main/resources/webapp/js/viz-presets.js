...
var addAggregatesOverTimeTpl = function(widgetsArray, entityName){
	...
	var config = getMasterSlaveConfig("raw", "Average "+entityName+" Duration (ms)", "Nb " + entityName + "s per minute");
	...
	var slave = new Widget(config.slaveid, new DefaultWidgetState(), new DashletState(config.slavetitle, false, 0, {}, new ChartOptions('stackedAreaChart', false, true), config.slaveconfig, new RTMAggBaseTemplatedQueryTmpl("tpm", "auto", overtimeFillBlanksTransform, "name"), new DefaultGuiClosed(), new DefaultInfo()));
