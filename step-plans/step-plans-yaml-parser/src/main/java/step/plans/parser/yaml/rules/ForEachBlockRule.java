...
public class ForEachBlockRule implements ArtefactFieldConversionRule {
    ...
    public JsonSchemaFieldProcessor getJsonSchemaFieldProcessor(JsonProvider jsonProvider) {
        return (objectClass, field, fieldMetadata, propertiesBuilder, requiredPropertiesOutput) -> {
            if (ForEachBlock.class.isAssignableFrom(objectClass)) {
                if (field.getName().equals(YamlPlanFields.FOR_BLOCK_DATA_SOURCE_TYPE_ORIGINAL_FIELD)) {
                    ...
                    List<String> dataSourceTypes = getAllDataSourceTypes();
                    ...
                    for (String dataSourceType : dataSourceTypes) {
                        ...
                        dataSourceFieldsYamlHelper.fillDataSourceJsonSchemaParams(dataSourceType, jsonProvider, dataSourceProperties);
    ...
    public YamlArtefactFieldDeserializationProcessor getArtefactFieldDeserializationProcessor() {
        return (artefactClass, field, output, codec) -> {
            if (artefactClass.equals(ForEachBlock.FOR_EACH_BLOCK_ARTIFACT_NAME) && YamlPlanFields.FOR_EACH_BLOCK_DATA_SOURCE_YAML_FIELD.equals(field.getKey())) {
                ...
                if (field.getValue() != null && field.getValue().isContainerNode()) {
                    ...
                    if (dataSourceProperties != null && dataSourceProperties.isContainerNode()) {
                        ...
                        while (fields.hasNext()) {
                            ...
                            ObjectNode dataSourceOutput = (ObjectNode) output.get(YamlPlanFields.FOR_BLOCK_DATA_SOURCE_ORIGINAL_FIELD);
                            if (isFileReference) {
    ...
    public YamlArtefactFieldSerializationProcessor getArtefactFieldSerializationProcessor() {
        return (artefact, field, fieldMetadata, gen) -> {
            if (ForEachBlock.class.isAssignableFrom(artefact.getClass())) {
                if (field.getName().equals(YamlPlanFields.FOR_BLOCK_DATA_SOURCE_ORIGINAL_FIELD)) {
                    String dataSourceType = ((ForEachBlock) artefact).getDataSourceType();
                    gen.writeFieldName(YamlPlanFields.FOR_EACH_BLOCK_DATA_SOURCE_YAML_FIELD);
